...
func _ready() -> void:
	...
	connect("tween_step", self, '_on_tween_step_with_curve')
	...
	for animation_data in _animation_data:
		...
		if animation_data.has('easing') and not animation_data.easing == null:
			if animation_data.easing is FuncRef:
				...
				easing_points = animation_data.easing
				_easing_curve = animation_data.easing
			else:
		...
		if easing_points is Array:
			...
			animation_data._use_step_callback = '_on_tween_step_with_curve'
		else:
...
func reset_data(strategy: int, play_mode: int, animation_length: float) -> void:
	var data: Array
...
func _apply_visibility_strategy(animation_data: Dictionary, strategy: int = Anima.VISIBILITY.IGNORE) -> void:
...
func _on_tween_step_with_easing(object: Object, key: NodePath, _time: float, elapsed: float) -> void:
...
func _on_tween_step_with_easing_callback(object: Object, key: NodePath, _time: float, elapsed: float) -> void:
	var index := _get_animation_data_index(key)
...
func _on_tween_step_with_easing_funcref(object: Object, key: NodePath, _time: float, elapsed: float) -> void:
	var index := _get_animation_data_index(key)
...
func _on_tween_step_with_curve(object: Object, key: NodePath, _time: float, elapsed: float) -> void:
	var index := _get_animation_data_index(key)

	if _animation_data[index]._use_step_callback != '_on_tween_step_with_curve':
		return
...
func _on_tween_step_without_easing(object: Object, key: NodePath, _time: float, elapsed: float) -> void:
	var index := _get_animation_data_index(key)
